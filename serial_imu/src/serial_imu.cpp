...
int main(int argc, char** argv)
{
	...
	std::string paramSerialPort;
	ros::param::get("~serial_port", paramSerialPort);

	
	std::string paramTopicName;
	ros::param::get("~imu_topic_name", paramTopicName);

	std::string paramRawTopicName;
	ros::param::get("~imu_raw_topic_name", paramRawTopicName);

	int paramBaud;
	ros::param::get("~serial_baud", paramBaud);

	ros::Publisher IMU_pub = n.advertise<sensor_msgs::Imu>(paramTopicName, 20);
	ros::Publisher Imu_0x91_pub = n.advertise<serial_imu::Imu_0x91_msg>(paramRawTopicName, 10);
	...
	sp.setPort(paramSerialPort);

	sp.setBaudrate(paramBaud);
	...
	{
		ROS_ERROR_STREAM(paramSerialPort << ": Unable to open." << " baudrate: "<<paramBaud);
		return -1;
	}
    
	if(sp.isOpen())
	{
		ROS_INFO_STREAM(paramSerialPort << ": Opened.");
	}
	else
	{
		return -1;
